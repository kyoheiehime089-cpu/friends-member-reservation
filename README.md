...
3. `stores`、`menus`、`plans`、`members`、`line_users`、`admin_users`、`reservation_slots`、`reservations`、`reservation_rules`、`notification_settings`、`audit_logs`、`settings_change_logs`、`mail_logs` が作成されます。
...
- `/admin/members`: 会員一覧、登録、プラン変更、ステータス管理、LINEユーザーのguest/member切り替え。
...
LINE公式アカウントのWebhook URLに `/api/line/webhook` を設定します。ユーザーが公式アカウントへメッセージを送ると、Webhookで `line_user_id` を受け取り、可能な場合はLINEプロフィールAPIで `display_name` を取得して `line_users` に保存します。初回の `member_status` は `guest` です。

管理者画面 `/admin/members` の「LINEユーザー」で該当ユーザーを「入会済みにする」と、`member_status` を `member` に更新し、`LINE_MEMBER_RICH_MENU_ID` のリッチメニューへ切り替えます。`guestに戻す` を押すと `LINE_GUEST_RICH_MENU_ID` のリッチメニューへ切り替えます。
