...
import { getPlanCategory, selectableBasePlans, selectedPlanIdsFromMemberPlan, selectExclusivePlanIds, type PlanLike } from '@/lib/planBundles';
...
export function MemberPlanManagerClient() {
  ...
  async function load() {
    try {
      ...
      setMessage('セミパーソナルとヨガは、それぞれ1つだけ選べます。セミナー・座学など別種類は組み合わせできます。');
    } catch (error) {
  ...
  async function save(member: Member) {
    ...
    try {
      const response = await adminFetch('/api/admin/member-plan', {
        method: 'PATCH',
        body: JSON.stringify({ memberId: member.id, planIds: drafts[member.id] ?? [], status: normalizeMemberStatus(member.status) })
      });
      const body = await response.json().catch(() => ({})) as ApiBody;
        ...
        <div className="grid gap-3">
          {filteredMembers.map((member) => {
            ...
            return <section key={member.id} className="rounded-3xl border bg-white p-4 shadow-sm">
              ...
              <div className="mt-3 grid gap-3">{groupedPlans.map(([category, categoryPlans]) => <div key={category} className="rounded-2xl border border-gray-200 p-3"><p className="mb-2 text-xs font-black text-gray-500">{category}{category === 'セミパーソナル' || category === 'ヨガ' ? 'はいずれか1つ' : 'は組み合わせ可'}</p><div className="grid gap-2 sm:grid-cols-2">{categoryPlans.map((plan) => <label key={plan.id} className={`rounded-xl border px-3 py-2 text-xs font-black ${draftIds.includes(plan.id) ? 'border-yellow-400 bg-yellow-50' : 'border-gray-200'}`}><input className="mr-2" type="checkbox" checked={draftIds.includes(plan.id)} onChange={() => toggle(member.id, plan.id)} />{plan.name}<span className="ml-1 text-gray-500">{planRule(plan)}</span></label>)}</div></div>)}</div>
            </section>;
          })}
          {filteredMembers.length === 0 && <div className="rounded-2xl bg-gray-50 p-5 text-center font-bold text-gray-500">該当する会員がいません。</div>}
        </div>
      </div>
    </AdminPage>
  );
