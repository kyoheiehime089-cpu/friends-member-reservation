...
export function OwnerCalendarFixed() {
  ...
  function openCell(day: Date, time: string, cellSlots: Slot[]) {
    ...
    const bookedIds = new Set(firstSlot ? active(firstSlot).map((reservation) => reservation.memberId).filter(Boolean) : []);
  ...
  async function book() {
    ...
    try {
      const response = await adminFetch(selectedSlot ? '/api/admin/book-slot' : '/api/admin/manual-reservation', { method: 'POST', body: JSON.stringify(body) });
