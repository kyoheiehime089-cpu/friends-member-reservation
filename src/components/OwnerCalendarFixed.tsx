...
type ApiBody = { ok?: boolean; message?: string; slots?: Slot[]; members?: Member[]; menus?: Menu[]; reservationId?: string; slotId?: string };
...
function active(slot: Slot) { return slot.reservations.filter((reservation) => reservation.status !== 'cancelled'); }
...
export function OwnerCalendarFixed() {
  ...
  const selectedAlreadyBooked = useMemo(() => {
    if (!selectedSlot || !selectedMember) return false;
    return active(selectedSlot).some((reservation) => reservation.memberId === selectedMember.id || (!!selectedMember.email && reservation.memberEmail === selectedMember.email));
  }, [selectedSlot, selectedMember]);
  const selectedSlotFull = selectedSlot ? Math.max(active(selectedSlot).length, selectedSlot.booked) >= selectedSlot.capacity : false;
  ...
  async function book() {
    ...
    try {
      ...
      setModalNotice(error instanceof Error ? error.message : '予約に失敗しました。');
    } finally {
      setBusy(false);
    }
  }
  ...
  async function cancelReservation(reservationId: string) {
    ...
    try {
      const response = await adminFetch('/api/admin/reservation-cancel', { method: 'POST', body: JSON.stringify({ reservationId }) });
      const result = await response.json().catch(() => ({})) as { ok?: boolean; message?: string };
      if (!response.ok || !result.ok) throw new Error(result.message ?? 'キャンセルに失敗しました。');
      setNotice(result.message ?? '予約をキャンセルしました。');
      setModal(null);
      await load().catch(() => undefined);
      window.setTimeout(() => window.location.reload(), 120);
    } catch (error) {
      setModalNotice(error instanceof Error ? error.message : 'キャンセルに失敗しました。');
    } finally {
      setBusy(false);
    }
  }
      ...
      <div className="space-y-3">
        <section className="rounded-3xl border bg-white p-3 shadow-sm">
          <div className="grid grid-cols-[auto_1fr_auto] items-center gap-2">
            ...
            <div className="text-center"><p className="text-lg font-black">{monthFmt.format(baseDate)}</p><p className="text-xs font-bold text-gray-500">{dateFmt.format(days[0])}〜{dateFmt.format(days[days.length - 1])}</p></div>
        ...
        {view === 'month' ? (
          <section className="rounded-2xl border bg-white p-1 shadow-sm"><div className="grid grid-cols-7 border-l border-t">{days.map((day) => { const list = slotsByDay.get(dateKey(day)) ?? []; return <button key={dateKey(day)} type="button" onClick={() => openCell(day, '09:00', list)} className="min-h-[92px] border-b border-r p-1 text-left"><p className="text-xs font-black">{dateFmt.format(day)}</p>{list.slice(0, 4).map((slot) => <div key={slot.id} className="mt-0.5 truncate rounded bg-gray-100 px-1 py-0.5 text-[10px] font-black text-gray-700">{timeKey(slot.startsAt)} {active(slot).map((row) => shortName(row.memberName)).join('、') || `空${slot.booked}/${slot.capacity}`}</div>)}</button>; })}</div></section>
        ) : (
          <section className="rounded-2xl border bg-white p-1 shadow-sm"><div className="w-full overflow-x-hidden"><div className="grid w-full border-l border-t" style={{ gridTemplateColumns: `42px repeat(${days.length}, minmax(0, 1fr))` }}><div className="border-b border-r p-1 text-center text-[10px] font-black">時間</div>{days.map((day) => <div key={dateKey(day)} className="border-b border-r p-1 text-center text-[10px] font-black">{dateFmt.format(day)}<br />{weekdayFmt.format(day)}</div>)}{times.map((time) => <div key={time} className="contents"><div className="min-h-[62px] border-b border-r p-1 text-center text-[10px] font-black">{time}</div>{days.map((day) => { const key = `${dateKey(day)}-${time}`; const list = slotMap.get(key) ?? []; return <button key={key} type="button" onClick={() => openCell(day, time, list)} className="min-h-[62px] overflow-hidden border-b border-r p-0.5 text-left">{list.map((slot) => { const activeRows = active(slot); if (!activeRows.length) return <div key={slot.id} className="rounded border border-dashed bg-gray-50 py-1 text-center text-[9px] font-bold text-gray-400">空{slot.booked}/{slot.capacity}</div>; return <div key={slot.id} className="mb-0.5 flex gap-0.5">{activeRows.map((row) => <div key={row.id} className={`min-h-[56px] flex-1 rounded px-0.5 py-1 text-[10px] font-black ${cellColor(slot.menuName)}`} style={{ writingMode: 'vertical-rl' }}>{shortName(row.memberName)}</div>)}</div>; })}</button>; })}</div>)}</div></div></section>
        )}

        {modal && (
          <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-3 sm:items-center"><div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-3xl bg-white p-4 shadow-xl"><div className="flex items-center justify-between"><p className="text-xl font-black">{modal.dateKey} {modal.time}</p><button type="button" onClick={() => setModal(null)} className="rounded-full border px-3 py-1 font-black">×</button></div><div className="mt-4 space-y-3">{modal.slots.map((slot) => <div key={slot.id} className="rounded-2xl border p-3"><p className="font-black">{timeKey(slot.startsAt)} {slot.menuName} {slot.booked}/{slot.capacity}名</p>{active(slot).length === 0 && <p className="mt-2 rounded-xl bg-gray-50 p-2 text-sm font-bold text-gray-500">予約者はいません。</p>}{active(slot).map((row) => <div key={row.id} className="mt-2 flex items-center justify-between gap-2 rounded-xl bg-gray-50 p-2"><span className="text-sm font-black">{row.memberName}</span><button type="button" disabled={busy} onClick={() => void cancelReservation(row.id)} className="rounded-full border border-red-300 px-3 py-1 text-xs font-black text-red-600">キャンセル</button></div>)}</div>)}<div className="rounded-2xl border border-yellow-200 bg-yellow-50 p-3"><p className="font-black">代理予約</p>{modalNotice && <p className="mt-2 rounded-xl bg-white p-2 text-sm font-black text-yellow-900">{modalNotice}</p>}<div className="mt-3 grid gap-2"><input value={memberSearch} onChange={(event) => setMemberSearch(event.target.value)} className="rounded-xl border px-3 py-2 font-bold" placeholder="会員名・メールで検索" /><select value={memberId} onChange={(event) => { setMemberId(event.target.value); setModalNotice(''); }} className="rounded-xl border px-3 py-2 font-bold" size={Math.min(Math.max(visibleMembers.length + 1, 3), 6)}><option value="">会員を選択</option>{visibleMembers.map((member) => <option key={member.id} value={member.id}>{member.full_name || member.email || member.id}</option>)}</select>{modal.slots.length > 0 && <select value={slotId} onChange={(event) => { setSlotId(event.target.value); setModalNotice(''); }} className="rounded-xl border px-3 py-2 font-bold"><option value="">新規枠を作る</option>{modal.slots.map((slot) => <option key={slot.id} value={slot.id}>{timeKey(slot.startsAt)} {slot.menuName} {slot.booked}/{slot.capacity}</option>)}</select>}{!slotId && <><select value={menuId} onChange={(event) => setMenuId(event.target.value)} className="rounded-xl border px-3 py-2 font-bold">{menus.map((menu) => <option key={menu.id} value={menu.id}>{menu.name}</option>)}</select><div className="grid grid-cols-2 gap-2"><input type="number" min="5" value={minutes} onChange={(event) => setMinutes(Number(event.target.value))} className="rounded-xl border px-3 py-2" /><input type="number" min="1" value={capacity} onChange={(event) => setCapacity(Number(event.target.value))} className="rounded-xl border px-3 py-2" /></div></>}<button type="button" disabled={busy || !memberId || selectedAlreadyBooked || selectedSlotFull} onClick={() => void book()} className="rounded-full bg-yellow-400 px-4 py-3 font-black disabled:opacity-50">{busy ? '予約中' : selectedAlreadyBooked ? 'この会員は予約済み' : selectedSlotFull ? '満席' : '予約を入れる'}</button></div></div></div></div></div>
        )}
      </div>
    </AdminPage>
  );
