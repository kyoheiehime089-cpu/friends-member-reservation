...
type ApiBody = { ok?: boolean; message?: string; slots?: Slot[]; members?: Member[]; menus?: Menu[]; reservationId?: string; slotId?: string; reservation?: { id?: string; status?: string; reservation_slot_id?: string | null; member_id?: string | null } };
...
function activeCount(slot: Slot) { return active(slot).length; }
...
function msg(error: unknown, fallback: string) { return error instanceof Error ? error.message : fallback; }
...
export function OwnerCalendarFixed() {
  ...
  useEffect(() => { void load().catch((error) => setNotice(msg(error, '読み込みに失敗しました。'))); }, [range.start.getTime(), range.end.getTime()]);
  ...
  const selectedSlotFull = selectedSlot ? activeCount(selectedSlot) >= selectedSlot.capacity : false;
  ...
  function addReservationToView(result: ApiBody, body: { memberId: string; slotId?: string; menuId?: string; date?: string; time?: string; minutes?: number; capacity?: number }) {
    ...
    const baseSlot: Slot = selectedSlot ?? {
      id: result.slotId || body.slotId || `temp-slot-${Date.now()}`,
      startsAt,
      endsAt,
    ...
    const reservation: Reservation = {
      id: result.reservationId || `temp-reservation-${Date.now()}`,
      status: 'booked',
    ...
    const updatedSlot = {
      ...baseSlot,
      reservations: [...baseSlot.reservations.filter((row) => row.status === 'booked' && row.memberId !== body.memberId), reservation]
    };
    updatedSlot.booked = activeCount(updatedSlot);
    setSlots((current) => {
      const exists = current.some((slot) => slot.id === updatedSlot.id);
      const next = exists ? current.map((slot) => slot.id === updatedSlot.id ? updatedSlot : slot) : [...current, updatedSlot];
      return next.sort((a, b) => String(a.startsAt).localeCompare(String(b.startsAt)));
    });
  }

  function removeReservationFromView(reservationId: string) {
    const remove = (slot: Slot) => {
      const reservations = slot.reservations.filter((row) => row.id !== reservationId && row.status === 'booked');
  ...
  async function book() {
    ...
    try {
      ...
      setNotice(`${result.message ?? '予約を入れました。'} 画面に反映しました。`);
      setModal(null);
      window.setTimeout(() => { void load().catch(() => undefined); }, 150);
    } catch (error) {
      setModalNotice(msg(error, '予約に失敗しました。'));
    } finally {
      setBusy(false);
    }
  }
  ...
  async function cancelReservation(reservationId: string) {
    if (busy) return;
    const target = modal?.slots.flatMap((slot) => active(slot)).find((reservation) => reservation.id === reservationId);
    if (!window.confirm(`${target?.memberName ?? 'この会員'}さんの予約を本当にキャンセルしますか？`)) return;
    setBusy(true);
    setModalNotice('キャンセルしています。');
    try {
      const result = await json('/api/admin/reservation-cancel', { method: 'POST', body: JSON.stringify({ reservationId }) });
      removeReservationFromView(reservationId);
      setNotice(`${result.message ?? '予約をキャンセルしました。'} 画面に反映しました。`);
      setModal(null);
      window.setTimeout(() => { void load().catch(() => undefined); }, 150);
    } catch (error) {
      setModalNotice(msg(error, 'キャンセルに失敗しました。'));
    } finally {
      setBusy(false);
    }
  }

  function closeModal() { setModal(null); setModalNotice(''); }
  ...
  return (
    <AdminPage title="予約カレンダー" description="管理者が予約状況確認・代理予約・キャンセルを行う画面です。">
      <div className="space-y-3">
        <section className="rounded-3xl border bg-white p-3 shadow-sm">
          <div className="grid grid-cols-[auto_1fr_auto] items-center gap-2"><button type="button" onClick={() => setBaseDate((date) => addMonths(date, -1))} className="rounded-full border px-3 py-2 text-xs font-black">前月</button><div className="text-center"><p className="text-lg font-black">{monthFmt.format(baseDate)}</p><p className="text-xs font-bold text-gray-500">{dateFmt.format(days[0])}〜{dateFmt.format(days[days.length - 1])}</p></div><button type="button" onClick={() => setBaseDate((date) => addMonths(date, 1))} className="rounded-full border px-3 py-2 text-xs font-black">次月</button></div>
          <div className="mt-3 grid grid-cols-3 gap-2"><button type="button" onClick={() => step(-1)} className="rounded-full border px-2 py-2 text-xs font-black">‹ {previousLabel}</button><button type="button" onClick={() => setBaseDate(today())} className="rounded-full bg-yellow-400 px-2 py-2 text-xs font-black">今日</button><button type="button" onClick={() => step(1)} className="rounded-full border px-2 py-2 text-xs font-black">{nextLabel} ›</button></div>
          <div className="mt-3 grid grid-cols-3 gap-2"><button type="button" onClick={() => setView('day')} className={`rounded-full px-2 py-2 text-xs font-black ${view === 'day' ? 'bg-gray-900 text-white' : 'border'}`}>1日</button><button type="button" onClick={() => setView('week')} className={`rounded-full px-2 py-2 text-xs font-black ${view === 'week' ? 'bg-gray-900 text-white' : 'border'}`}>1週間</button><button type="button" onClick={() => setView('month')} className={`rounded-full px-2 py-2 text-xs font-black ${view === 'month' ? 'bg-gray-900 text-white' : 'border'}`}>1ヶ月</button></div>
          {notice && <p className="mt-3 rounded-2xl bg-green-50 p-3 text-sm font-black text-green-800">{notice}</p>}
        </section>
        {view === 'month' ? <section className="rounded-2xl border bg-white p-1 shadow-sm"><div className="grid grid-cols-7 border-l border-t">{days.map((day) => { const list = slotsByDay.get(dateKey(day)) ?? []; return <button key={dateKey(day)} type="button" onClick={() => openCell(day, '09:00', list)} className="min-h-[92px] border-b border-r p-1 text-left"><p className="text-xs font-black">{dateFmt.format(day)}</p>{list.slice(0, 4).map((slot) => <div key={slot.id} className="mt-0.5 truncate rounded bg-gray-100 px-1 py-0.5 text-[10px] font-black text-gray-700">{timeKey(slot.startsAt)} {active(slot).map((row) => shortName(row.memberName)).join('、') || `空${slot.booked}/${slot.capacity}`}</div>)}</button>; })}</div></section> : <section className="rounded-2xl border bg-white p-1 shadow-sm"><div className="w-full overflow-x-hidden"><div className="grid w-full border-l border-t" style={{ gridTemplateColumns: `42px repeat(${days.length}, minmax(0, 1fr))` }}><div className="border-b border-r p-1 text-center text-[10px] font-black">時間</div>{days.map((day) => <div key={dateKey(day)} className="border-b border-r p-1 text-center text-[10px] font-black">{dateFmt.format(day)}<br />{weekdayFmt.format(day)}</div>)}{times.map((time) => <div key={time} className="contents"><div className="min-h-[62px] border-b border-r p-1 text-center text-[10px] font-black">{time}</div>{days.map((day) => { const list = slotMap.get(`${dateKey(day)}-${time}`) ?? []; return <button key={`${dateKey(day)}-${time}`} type="button" onClick={() => openCell(day, time, list)} className="min-h-[62px] overflow-hidden border-b border-r p-0.5 text-left">{list.map((slot) => { const activeRows = active(slot); if (!activeRows.length) return <div key={slot.id} className="rounded border border-dashed bg-gray-50 py-1 text-center text-[9px] font-bold text-gray-400">空{slot.booked}/{slot.capacity}</div>; return <div key={slot.id} className="mb-0.5 flex gap-0.5">{activeRows.map((row) => <div key={row.id} className={`min-h-[56px] flex-1 rounded px-0.5 py-1 text-[10px] font-black ${cellColor(slot.menuName)}`} style={{ writingMode: 'vertical-rl' }}>{shortName(row.memberName)}</div>)}</div>; })}</button>; })}</div>)}</div></div></section>}
        {modal && <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-3 sm:items-center" onClick={(event) => { if (event.target === event.currentTarget) closeModal(); }}><div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-3xl bg-white p-0 shadow-xl" onClick={(event) => event.stopPropagation()}><div className="sticky top-0 z-10 flex items-center justify-between border-b bg-white p-4"><p className="text-xl font-black">{modal.dateKey} {modal.time}</p><button type="button" aria-label="閉じる" onClick={closeModal} className="rounded-full border px-3 py-1 font-black">×</button></div><div className="p-4"><div className="mt-1 space-y-3">{modal.slots.map((slot) => <div key={slot.id} className="rounded-2xl border p-3"><p className="font-black">{timeKey(slot.startsAt)} {slot.menuName} {activeCount(slot)}/{slot.capacity}名</p>{active(slot).length === 0 && <p className="mt-2 rounded-xl bg-gray-50 p-2 text-sm font-bold text-gray-500">予約者はいません。</p>}{active(slot).map((row) => <div key={row.id} className="mt-2 flex items-center justify-between gap-2 rounded-xl bg-gray-50 p-2"><span className="text-sm font-black">{row.memberName}</span><button type="button" disabled={busy} onClick={() => void cancelReservation(row.id)} className="rounded-full border border-red-300 px-3 py-1 text-xs font-black text-red-600">キャンセル</button></div>)}</div>)}<div className="rounded-2xl border border-yellow-200 bg-yellow-50 p-3"><p className="font-black">代理予約</p>{modalNotice && <p className="mt-2 rounded-xl bg-white p-2 text-sm font-black text-yellow-900">{modalNotice}</p>}<div className="mt-3 grid gap-2"><input value={memberSearch} onChange={(event) => setMemberSearch(event.target.value)} className="rounded-xl border px-3 py-2 font-bold" placeholder="会員名・メールで検索" /><select value={memberId} onChange={(event) => { setMemberId(event.target.value); setModalNotice(''); }} className="rounded-xl border px-3 py-2 font-bold" size={Math.min(Math.max(visibleMembers.length + 1, 3), 6)}><option value="">会員を選択</option>{visibleMembers.map((member) => <option key={member.id} value={member.id}>{member.full_name || member.email || member.id}</option>)}</select>{modal.slots.length > 0 && <select value={slotId} onChange={(event) => { setSlotId(event.target.value); setModalNotice(''); }} className="rounded-xl border px-3 py-2 font-bold"><option value="">新規枠を作る</option>{modal.slots.map((slot) => <option key={slot.id} value={slot.id}>{timeKey(slot.startsAt)} {slot.menuName} {activeCount(slot)}/{slot.capacity}</option>)}</select>}{!slotId && <><select value={menuId} onChange={(event) => setMenuId(event.target.value)} className="rounded-xl border px-3 py-2 font-bold">{menus.map((menu) => <option key={menu.id} value={menu.id}>{menu.name}</option>)}</select><div className="grid grid-cols-2 gap-2"><input type="number" min="5" value={minutes} onChange={(event) => setMinutes(Number(event.target.value))} className="rounded-xl border px-3 py-2" /><input type="number" min="1" value={capacity} onChange={(event) => setCapacity(Number(event.target.value))} className="rounded-xl border px-3 py-2" /></div></>}<button type="button" disabled={busy || !memberId || selectedAlreadyBooked || selectedSlotFull} onClick={() => void book()} className="rounded-full bg-yellow-400 px-4 py-3 font-black disabled:opacity-50">{busy ? '保存中' : selectedAlreadyBooked ? 'この会員は予約済み' : selectedSlotFull ? '満席' : '予約を入れる'}</button></div></div></div></div></div></div>}
      </div>
    </AdminPage>
  );
}
