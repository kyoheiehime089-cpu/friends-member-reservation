...
type ApiBody = { ok?: boolean; message?: string; slots?: Slot[]; members?: Member[]; menus?: Menu[]; reservationId?: string; slotId?: string; memberLabel?: string; reservation?: { reservation_slot_id?: string | null; member_id?: string | null } };
...
export function OwnerCalendarOperational() {
  ...
  function upsertReservation(result: ApiBody, body: { memberId: string; slotId?: string; menuId?: string; date?: string; time?: string; minutes?: number; capacity?: number }) {
    ...
    setSlots((current) => {
      ...
      const updatedSlot: Slot = {
        ...baseSlot,
        id: realSlotId,
        reservations: [...baseSlot.reservations.filter((row) => row.status === 'booked' && row.memberId !== body.memberId && row.id !== reservationId), reservation]
      };
