...
type ApiBody = { ok?: boolean; message?: string; slots?: Slot[]; slot?: Slot; members?: Member[]; menus?: Menu[]; reservationId?: string; slotId?: string; memberLabel?: string; deletedSlotId?: string | null; reservation?: { reservation_slot_id?: string | null; member_id?: string | null } };
...
function dateKey(value: Date | string | null) { return value ? keyFmt.format(typeof value === 'string' ? new Date(value) : value) : ''; }
function timeKey(value: string | null) { return value ? timeFmt.format(new Date(value)) : ''; }
...
function keepEmptySlot(menuName: string) { return menuName.includes('セミパーソナル') || menuName.includes('ヨガ'); }
...
export function OwnerCalendarFinal() {
  ...
  function upsertReservation(result: ApiBody, body: { memberId: string; slotId?: string; menuId?: string; date?: string; time?: string; minutes?: number; capacity?: number }) {
    ...
    setSlots((current) => {
      ...
      const baseSlot: Slot = existingSlot || { id: realSlotId, startsAt, endsAt, menuName: menu?.name || 'メニュー未設定', capacity: Number(body.capacity || menu?.default_capacity || 5), booked: 0, isOpen: true, reservations: [] };
  ...
  function removeReservation(reservationId: string, slotIdValue?: string | null, memberIdValue?: string | null, deletedSlotId?: string | null) {
    ...
    const filterEmpty = (slot: Slot) => {
      if (deletedSlotId && slot.id === deletedSlotId) return false;
      if (slotIdValue && slot.id === slotIdValue && active(slot).length === 0 && !keepEmptySlot(slot.menuName)) return false;
      return true;
    };
    setSlots((current) => current.map(removeFromSlot).filter(filterEmpty));
    setModal((current) => current ? { ...current, slots: current.slots.map(removeFromSlot).filter(filterEmpty) } : current);
  }
  ...
  async function cancelReservation(reservationId: string, slotIdValue?: string | null, memberIdValue?: string | null) {
    ...
    try {
      ...
      removeReservation(reservationId, result.reservation?.reservation_slot_id || slotIdValue, result.reservation?.member_id || memberIdValue, result.deletedSlotId);
      setModal(null);
