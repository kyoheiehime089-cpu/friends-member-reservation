...
type CreateResponse = { ok?: boolean; message?: string; detail?: string; memberMail?: string; memberMailError?: string | null };
...
function bookingSuccessMessage(result: CreateResponse) { if (result.memberMail === 'sent') return '予約が完了しました。予約完了メールも送信しました。'; if (result.memberMail === 'failed') return `予約は完了しましたが、予約完了メールの送信に失敗しました。${result.memberMailError ? `\n\n原因: ${result.memberMailError}` : ''}`; return '予約が完了しました。予約一覧に移動します。'; }
...
export default function ReservePage() {
  ...
  const load = useCallback(async (preserveFeedback = false) => {
    ...
    const nextMenus = (menuRows ?? []).map((menu) => ({ id: menu.id, name: menu.name, description: menu.description, capacity: menu.default_capacity })).sort((a, b) => menuOrder(a.name) - menuOrder(b.name));
    setMenus(nextMenus);
    const activeMenuId = nextMenus.some((menu) => menu.id === selectedMenuId) ? selectedMenuId : '';
    if (selectedMenuId && !activeMenuId) setSelectedMenuId('');
  ...
  const reserve = async (slotId: string) => {
    ...
    } catch (error) { show('error', friendly(error instanceof Error ? error.message : '通信エラーが発生しました。')); setSubmittingSlotId(null); void load(true); }
  };
  ...
  return (
    <AppShell>
      <div className="space-y-4">
        ...
        {showMenuPicker ? (
          <>
            <div className="space-y-1"><h1 className="text-3xl font-black">予約する</h1><p className="text-sm font-semibold text-gray-600">まず予約したいメニューを選んでください。</p></div>
            ...
            <section className="rounded-3xl border border-gray-200 bg-white p-4 shadow-sm"><div className="space-y-3">{loading && <div className="rounded-2xl bg-gray-50 p-5 text-center text-sm font-bold text-gray-600">メニューを読み込んでいます。</div>}{!loading && menus.map((menu) => (<button key={menu.id} type="button" onClick={() => selectMenu(menu.id)} className="flex w-full items-center justify-between gap-4 rounded-3xl border border-gray-200 bg-white p-5 text-left shadow-sm transition hover:border-yellow-300 hover:bg-yellow-50"><div className="min-w-0 flex-1"><p className="text-2xl font-black text-gray-950">{menu.name}</p><p className="mt-1 text-sm font-bold text-gray-500">定員目安 {menu.capacity}名</p>{menu.description && <p className="mt-2 text-xs font-semibold text-gray-500">{menu.description}</p>}</div><span className="flex-shrink-0 whitespace-nowrap rounded-full bg-yellow-400 px-4 py-2 text-center text-sm font-black text-gray-950">選択</span></button>))}{!loading && menus.length === 0 && <div className="rounded-2xl bg-gray-50 p-5 text-center text-sm font-bold text-gray-600">選択できるメニューがありません。</div>}</div></section>
          </>
        ) : (
          <>
            <div className="flex items-start justify-between gap-3"><div><h1 className="text-2xl font-black">{selectedMenu?.name} の予約</h1><p className="text-xs font-bold text-gray-500">{rangeLabel}</p></div><button type="button" onClick={resetMenu} className="rounded-full border border-gray-300 bg-white px-4 py-2 text-xs font-black text-gray-700 shadow-sm">メニュー変更</button></div>
            {feedback && <div className={`whitespace-pre-line rounded-2xl border p-4 text-sm font-bold ${className(feedback.kind)}`}>{feedback.text}</div>}
            <section className="rounded-3xl border border-gray-200 bg-white p-3 shadow-sm"><div className="mb-3 grid grid-cols-4 gap-2"><button type="button" onClick={() => setOffset((current) => Math.max(0, current - (mode === 'threeDays' ? 3 : 7)))} disabled={offset === 0} className="rounded-full border border-gray-300 px-2 py-2 text-xs font-black disabled:cursor-not-allowed disabled:opacity-40">前</button><button type="button" onClick={() => setDisplayMode('threeDays')} className={`rounded-full px-2 py-2 text-xs font-black ${mode === 'threeDays' ? 'bg-yellow-400 text-gray-950' : 'border border-gray-300 text-gray-700'}`}>3日</button><button type="button" onClick={() => setDisplayMode('week')} className={`rounded-full px-2 py-2 text-xs font-black ${mode === 'week' ? 'bg-yellow-400 text-gray-950' : 'border border-gray-300 text-gray-700'}`}>1週間</button><button type="button" onClick={() => setOffset((current) => current + (mode === 'threeDays' ? 3 : 7))} className="rounded-full border border-gray-900 px-2 py-2 text-xs font-black">次</button></div>{!loading && <ReservationGrid dense dates={dates} slots={slots} submittingSlotId={submittingSlotId} timeLabels={standardTimes} onReserve={reserve} />}{loading && <div className="rounded-2xl border border-gray-100 bg-gray-50 p-5 text-center text-sm font-bold text-gray-600">予約枠を読み込んでいます。</div>}</section>
