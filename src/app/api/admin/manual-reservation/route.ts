...
async function getCalendarSlot(db: ReturnType<typeof createServiceClient>, slotId: string) {
  ...
  const menuResult = slot.menu_id ? await db.from('menus').select('id,name').eq('id', slot.menu_id).maybeSingle() : { data: null, error: null };
  ...
  const reservations = effectiveBookedReservations((reservationRows ?? []) as ReservationStateRow[]);
  const memberIds = Array.from(new Set(reservations.map((reservation) => reservation.member_id).filter(Boolean))) as string[];
  const members: MemberRow[] = memberIds.length ? (((await db.from('members').select('id,full_name,email,plan_id').in('id', memberIds)).data ?? []) as MemberRow[]) : [];
  const planIds = Array.from(new Set(members.map((member) => member.plan_id).filter(Boolean))) as string[];
  const plans: PlanRow[] = planIds.length ? (((await db.from('plans').select('id,name').in('id', planIds)).data ?? []) as PlanRow[]) : [];
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ ok: true, reservationId: result.reservationId, slotId: result.slotId, memberLabel: result.memberLabel, slot, message: `${result.memberLabel}さんの予約を入れました。` });
  } catch (error) {
