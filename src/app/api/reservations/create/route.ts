...
function parts(value: Date | string) {
  ...
  return {
    y: Number(p.find((x) => x.type === 'year')?.value ?? '0'),
    m: Number(p.find((x) => x.type === 'month')?.value ?? '0'),
    d: Number(p.find((x) => x.type === 'day')?.value ?? '0')
  };
}
...
function weekRange(value: Date | string) {
  ...
  return {
    start: new Date(Date.UTC(p.y, p.m - 1, p.d - diff, -9, 0, 0, 0)),
    end: new Date(Date.UTC(p.y, p.m - 1, p.d - diff + 7, -9, 0, 0, 0))
  };
}

function deadline(value: Date | string) {
  const p = parts(value);
...
export async function POST(request: Request) {
  ...
  if ((bookedRows?.length ?? 0) >= slot.capacity) return NextResponse.json({ ok: false, message: 'この枠は満席になりました。別の枠をお選びください。' }, { status: 409 });
